...
public class Calculator {
    ...
    public Object calculate() {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            strings[i] = tokens.get(i).getVal().toString();
        }
    ...
    private void calc(List<Integer> priorities) {
        ...
        if (val instanceof Integer) {
            tokens.set(max, new Token("int", (int)val));
            priorities.set(max, 0);
            remove(max, priorities);
            remove(max, tokens);
        }
        if (val instanceof Double) {
            tokens.set(max, new Token("num", (double)val));
            priorities.set(max, 0);
            remove(max, priorities);
            remove(max, tokens);
        }
        if (val instanceof Long) {
            tokens.set(max, new Token("lon", (long)val));
            priorities.set(max, 0);
            remove(max, priorities);
            remove(max, tokens);
        }
